$(document).ready(function(){
	...
	function LoadRole(){
		$.ajax({
			// type: 'POST',
			url: '../../backbone/controller/getRole.php',
			data: "get_role",
	...
	function LoadCategories(){
		...
		$("#send").click(function(e){
			$('#createUser').validate({
				...
				$.ajax({
					type: 'POST',
					url: '../../backbone/controller/userRegistration.php',
					data: data,
					beforeSend: function(){
					...
					success: function(response){
						if (response == 'created'){
							...
							$("#createUser")[0].reset();
							$(".modal").modal('hide');
							toastr.success('User Successfully Created','Success Alert',{timeOut:20000});
						}else{
							$("#send").html('<i class="fa fa-save"></i>&ensp;Create');
							get_table_data();
							$("#send").attr("disabled", false);
		...
		function get_table_data(){
			...
			$.ajax({
				url: "../../backbone/controller/getUsers.php",
				type: "POST",
				data: 'getUsers',
				dataType: 'json',
				beforeSend: function(){
		...
		function manage_data(data){
				...
				$.each(data, function(key, value){
					...
					row += '<td class="text-center upper">'+ value.userName +'</td> ';  
					row += '<td class="text-center upper">'+ value.userRole +'</td>';
					if(value.status == 'active'){
						row += '<td class="text-center upper"><span class="label label-success">'+ value.status +'</span></td>';
					}else if(value.status == 'inactive'){
						row += '<td class="text-center upper"><span class="label label-warning">'+ value.status +'</span></td>';
					}
					row += '<td class="text-center upper" data-id="'+ value.id +'">'; 
					row += ' <button id="activate" class="btn btn-small btn-success btn-xs"><i class="fa fa-check"></i></button> '; 
					row += ' <button id="deactivate" class="btn btn-small btn-warning btn-xs"><i class="fa fa-times"></i></button> ';
		...
		$("body").on("click","#activate", function(){
			...
			$.ajax({
				type: 'POST',
				url: '../../backbone/controller/activateUser.php',
				data: {id:id},
				beforeSend: function(){
						$("#send").html('Submitting Leave..');
						$("#send").attr("disabled", true);
				...
				success: function(response){
					if (response == 'activated'){
						get_table_data();
						$("#activate").html('<i class="fa fa-save"></i>&ensp;Update');
						$("#activate").attr("disabled", false);
						toastr.success('Successfully Activated','Success Alert',{timeOut:20000});
					}else{
						get_table_data();
						$("#activate").html('<i class="fa fa-save"></i>&ensp;Update');
						$("#activate").attr("disabled", false);
		...
		$("body").on("click","#deactivate", function(){
			...
			$.ajax({
				type: 'POST',
				url: '../../backbone/controller/deactivateUser.php',
				data: {id:id},
				beforeSend: function(){
						$("#send").html('Submitting Leave..');
						$("#send").attr("disabled", true);
				...
				success: function(response){
					if (response == 'deactivate'){
						get_table_data();
						$("#deactivate").html('<i class="fa fa-save"></i>&ensp;Update');
						$("#deactivate").attr("disabled", false);
						toastr.success('Successfully Deactivated','Success Alert',{timeOut:20000});
					}else{
						get_table_data();
						$("#deactivate").html('<i class="fa fa-save"></i>&ensp;Update');
						$("#deactivate").attr("disabled", false);
		...
		$("body").on("click","#delete", function(){
			...
			$.ajax({
				type: 'POST',
				url: '../../backbone/controller/deleteUsers.php',
				data: {id:id},
				beforeSend: function(){
